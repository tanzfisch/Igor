...
void UserControlMesh::initGUI()
{
	...
	iWidgetButton* bakeButton = static_cast<iWidgetButton*>(iWidgetManager::getInstance().createWidget("Button"));
	_allWidgets.push_back(bakeButton);
...
void UserControlMesh::onBakeAction(iWidget* source)
{
	...
	scene->getRoot()->getChild("ModelViewerRoot")->insertNode(newMeshNode);
}
