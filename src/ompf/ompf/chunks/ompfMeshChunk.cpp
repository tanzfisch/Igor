...
namespace OMPF
{
    ...
    bool ompfMeshChunk::write(std::ofstream &file, const ompfSettings &settings)
    {
        ...
        con_debug("---------------------------------------------------");
        con_debug("write ompfMeshChunk " << this->getName());
        ...
        while (iterTex != _textures.end())
        {
            ...
            con_debug("texture " << i << " " << (*iterTex).second);
        ...
        con_debug("vertexCount " << _vertexCount << " vertexDataSize " << _vertexDataSize);

        iaSerializable::writeUInt32(file, _indexCount);
        iaSerializable::write(file, static_cast<char *>(_indexData), _indexDataSize);
        con_debug("indexCount " << _indexCount << " indexDataSize " << _indexDataSize);
    ...
    {
        con_debug("---------------------------------------------------");
        con_debug("read ompfMeshChunk " << this->getName());
        ...
        con_debug("meshType " << static_cast<int>(_meshType));

        uint8 textureCount = 0;
        iaSerializable::readUInt8(file, textureCount);
        con_debug("textureCount " << textureCount);
