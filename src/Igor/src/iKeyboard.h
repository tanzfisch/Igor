...
namespace Igor
{
	...
	*/
	iaEVENT(KeyDownEvent, KeyDownDelegate, void, (), ());

	/*! key down event ext

	will be called for every key
	*/
	iaEVENT(KeyDownEventExt, KeyDownDelegateExt, void, (iKeyCode key), (key));

	/*! key up event

	will be called for a specific single key
	*/
	iaEVENT(KeyUpEvent, KeyUpDelegate, void, (), ());

	/*! key up event ext

	will be called for every key
	*/
	iaEVENT(KeyUpEventExt, KeyUpDelegateExt, void, (iKeyCode key), (key));
	
	/*! ASCII stream event

	will be called every time a ascii value was generated by the keyboard driver
	*/
	iaEVENT(KeyASCIIEvent, KeyASCIIDelegate, void, (uint8 c), (c));
	...
	*/
	...
	private:
		...
		*/
        KeyDownEvent _keyDownEvent[static_cast<uint32>(iKeyCode::KeyCodeCount)];

		/*! holds one key up event for every possible key code
		*/
        KeyUpEvent _keyUpEvent[static_cast<uint32>(iKeyCode::KeyCodeCount)];

		/*! the ascii event
		*/
		KeyASCIIEvent _keyASCIIEvent;

		/*! one key down event for all possible key codes
		*/
		KeyDownEventExt _keyDownEventExt;

		/*! one key up event for all possible key codes
		*/
		KeyUpEventExt _keyUpEventExt;											
	...
	public:
		...
		*/
		void registerKeyDownDelegate(KeyDownDelegate keyDownDelegate, iKeyCode key);
		...
		*/
		void unregisterKeyDownDelegate(KeyDownDelegate keyDownDelegate, iKeyCode key);
		...
		\param key key the event is triggered by
		*/
		void registerKeyUpDelegate(KeyUpDelegate keyUpDelegate, iKeyCode key);
		...
		\param key the event was triggered by
		*/
		void unregisterKeyUpDelegate(KeyUpDelegate keyUpDelegate, iKeyCode key);
		...
		*/
		void registerKeyDownDelegate(KeyDownDelegateExt keyDownDelegate);

		/*! unregister delegate from any key down event

		\param keyDownDelegate the delegate to unregister
		*/
		void unregisterKeyDownDelegate(KeyDownDelegateExt keyDownDelegate);
