...
#define GL_CHECK_ERROR() 1
	...
	void iRenderer::init()
	{
		if (!_initialized)
		{
			glEnable(GL_LINE_SMOOTH); GL_CHECK_ERROR();
			glEnable(GL_POINT_SMOOTH); GL_CHECK_ERROR();
			...
			glHint(GL_POLYGON_SMOOTH_HINT, GL_DONT_CARE);	GL_CHECK_ERROR();
	...
	void iRenderer::enableStencilTest(bool enable)
	{
		...
		{
			glDisable(GL_STENCIL_TEST); GL_CHECK_ERROR();
		}
	}
