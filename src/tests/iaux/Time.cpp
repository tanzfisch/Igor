...
{
    iaTime time;
    IAUX_EXPECT_EQUAL(time.getMicrosenconds(), 0);
...
{
    iaTime time(100);
    IAUX_EXPECT_EQUAL(time.getMicrosenconds(), 100);
...
IAUX_TEST(TimeTests, FromSeconds)
{
    ...
    IAUX_EXPECT_EQUAL(time.getMicrosenconds(), static_cast<uint64>(12.3 * 1000000));
...
IAUX_TEST(TimeTests, FromMilliseconds)
{
    ...
    IAUX_EXPECT_EQUAL(time.getMicrosenconds(), 1234 * 1000);
...
IAUX_TEST(TimeTests, Operators)
{
    ...
    time1 += time2;
    IAUX_EXPECT_EQUAL(time1.getMicrosenconds(), 424);
    IAUX_EXPECT_EQUAL(time1.getMilliseconds(), 424.0 / 1000.0);
    IAUX_EXPECT_EQUAL(time1.getSeconds(), 424.0 / 1000000.0);

    iaTime time3(1000);
    time1 -= time3;
    IAUX_EXPECT_EQUAL(time1.getMicrosenconds(), -576);

    time2 = time1 + time3;
    IAUX_EXPECT_EQUAL(time2.getMicrosenconds(), 424);

    time1 = time2 - time3;
    IAUX_EXPECT_EQUAL(time1.getMicrosenconds(), -576);

    time1 *= 2;
    IAUX_EXPECT_EQUAL(time1.getMicrosenconds(), -1152);

    time1 = time3 * 0.5;
    IAUX_EXPECT_EQUAL(time1.getMicrosenconds(), 500);
}
