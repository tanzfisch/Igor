...
void main()
{
	...
	if(N.y > 0)
	{
		diffuseTextureColor1 = texture2D(igor_texture1, P.xz * scale).rgb * texSelector.y;
	}
	else
	{
		diffuseTextureColor1 = texture2D(igor_texture2, P.xz * scale).rgb * texSelector.y;
	}
	
	vec3 diffuseTextureColor0 = texture2D(igor_texture0, P.yz * scale).rgb * texSelector.x;
	vec3 diffuseTextureColor2 = texture2D(igor_texture0, P.xy * scale).rgb * texSelector.z;
