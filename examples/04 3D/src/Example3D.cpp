...
void Example3D::init()
{
    ...
    iNodeTransform* allObjectsHeading = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    ...
    iNodeTransform* allObjectsPitch = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    ...
    iNodeTransform* catTransform = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    catTransform->setName("cat transform");
    catTransform->translate(1, -1, 0);
    iNodeModel* catModel = static_cast<iNodeModel*>(iNodeManager::getInstance().createNode(iNodeType::iNodeModel));
    catModel->setModel("cat.ompf");
    // add the cat to the cat transform
    catTransform->insertNode(catModel);
    // repead for the teapot
    iNodeTransform* teapotTransform = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    teapotTransform->setName("teapot transform");
    teapotTransform->translate(1, -1, 0);
    teapotTransform->scale(0.33, 0.33, 0.33);
    iNodeModel* teapotModel = static_cast<iNodeModel*>(iNodeManager::getInstance().createNode(iNodeType::iNodeModel));
    teapotModel->setModel("teapot.ompf");
    teapotTransform->insertNode(teapotModel);
    // repeat for a crate
    iNodeTransform* crateTransform = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    crateTransform->setName("crate transform");
    crateTransform->translate(1, -0.5, 0);
    iNodeModel* crateModel = static_cast<iNodeModel*>(iNodeManager::getInstance().createNode(iNodeType::iNodeModel));
    ...
    // the teapot as LOD0
    iNodeTransform* lod0Transform = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    lod0Transform->setName("lod0 transform");
    lod0Transform->translate(-1, -1, 0);
    lod0Transform->scale(0.33, 0.33, 0.33);
    iNodeModel* lod0Model = static_cast<iNodeModel*>(iNodeManager::getInstance().createNode(iNodeType::iNodeModel));
    lod0Model->setModel("teapot.ompf");
    lod0Transform->insertNode(lod0Model);
    // the cat as LOD1
    iNodeTransform* lod1Transform = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    lod1Transform->setName("lod1 transform");
    lod1Transform->translate(-1, -1, 0);
    iNodeModel* lod1Model = static_cast<iNodeModel*>(iNodeManager::getInstance().createNode(iNodeType::iNodeModel));
    lod1Model->setModel("cat.ompf");
    lod1Transform->insertNode(lod1Model);
    // the create as LOD2
    iNodeTransform* lod2Transform = static_cast<iNodeTransform*>(iNodeManager::getInstance().createNode(iNodeType::iNodeTransform));
    lod2Transform->setName("lod0 transform");
    lod2Transform->translate(-1, -0.5, 0);
    iNodeModel* lod2Model = static_cast<iNodeModel*>(iNodeManager::getInstance().createNode(iNodeType::iNodeModel));
