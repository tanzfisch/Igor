...
int main()
{
	...
	window->setTitle("Igor - Physics Example");
	window->setClientSize(1024, 768);
	window->setCentered();
	window->open();

	iProject::getInstance().load("project/example_physics.project");

	// create example and add it as layer to the application
	igor::iApplication::getInstance().addLayer(new PhysicsExample(window));
