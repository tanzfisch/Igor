...
#else
    getView().setScene(iProject::getInstance().getActiveScene());
...
{
    // TODO getView().setScene(iProject::getInstance().getActiveScene());
...
bool Example3D::onKeyDown(iEventKeyDown &event)
{
    ...
    case iKeyCode::Space:
    {
        ...
        auto scene = iProject::getInstance().getActiveScene();
